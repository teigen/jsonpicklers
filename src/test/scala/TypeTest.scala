...
class TypeTest extends PropSpec with GeneratorDrivenPropertyChecks {
  ...
  property("property"){
    forAll(arbitrary[String].suchThat(_.size > 1), arbitrary[Int]){ (name, value) =>
  ...
  property("~"){
    ...
    forAll(names, arbitrary[Int], arbitrary[Int]){ (names, aVal, bVal) =>
  ...
  property("option"){
    forAll(arbitrary[String].suchThat(_.size > 0), arbitrary[Option[Int]]){ (name, value) =>
